...
import app.cloud.ccwb.cn.linlibrary.qrscan.camera.FrontLightMode;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (isSupportZoom && cameraManager.isOpen()) {
            ...
            if (event.getPointerCount() > 1) {
                int i = event.getAction() & MotionEvent.ACTION_MASK;//多点触控
                if (i == MotionEvent.ACTION_POINTER_DOWN) {
