package app.cloud.ccwb.cn.linlibrary.edit;
...
public class PayPsdInputView extends EditText {
    ...
    protected void onDraw(Canvas canvas) {
        //不删除的画会默认绘制输入的文字
//       super.onDraw(canvas);
        if (psdType == psdType_weChat) {
            drawWeChatBorder(canvas);
//                drawItemFocused(canvas, position);
        } else if (psdType == psdType_bottomLine) {
...
//            canvas.drawPosText("*",new float[]{startX + i * 2 * startX ,height/4 +startY},circlePaint);
        }
    }
    ...
    private void drawStar(Canvas canvas) {
        ...
        for (int i = 0; i < textLength; i++) {
            ...
            circlePaint.setTextAlign(Paint.Align.CENTER);
            canvas.drawPosText("✽", new float[]{startX + i * 2 * startX, height / 8 + startY}, circlePaint);
        }
    }
