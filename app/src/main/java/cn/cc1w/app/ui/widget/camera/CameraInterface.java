package cn.cc1w.app.ui.widget.camera;
...
import cn.cc1w.app.ui.widget.camera.util.AngleUtil;
import cn.cc1w.app.ui.widget.camera.util.CameraParamUtil;
import cn.cc1w.app.ui.widget.camera.util.CheckPermission;
import cn.cc1w.app.ui.widget.camera.util.DeviceUtil;
import cn.cc1w.app.ui.widget.camera.util.FileUtil;
import cn.cc1w.app.ui.widget.camera.util.KLog;
import cn.cc1w.app.ui.widget.camera.util.ScreenUtils;
...
public class CameraInterface implements Camera.PreviewCallback {
    ...
    public void setZoom(float zoom, int type) {
        ...
        if (type == TYPE_RECORDER) {//如果不是录制视频中，上滑不会缩放
            if (!isRecorder) {
                return;
            }
            if (zoom >= 0) {
            ...
            KLog.i(TAG, "setZoom = " + nowScaleRate);
        }
    ...
    private synchronized void openCamera(int id) {
        ...
        if (Build.VERSION.SDK_INT > 17 && this.mCamera != null) {
            try {
                ...
                e.printStackTrace();
                KLog.e("CJT", "enable shutter sound faild");
            }
        }
    }
...
//        mCamera = Camera.open();
    ...
    public void doStartPreview(SurfaceHolder holder, float screenProp) {
        if (isPreviewing) {
            KLog.i(TAG, "doStartPreview isPreviewing");
        }
...
//                destroyCameraInterface();
                KLog.i(TAG, "=== Destroy Camera ===");
            } catch (IOException e) {
                e.printStackTrace();
            }
        } else {
            KLog.i(TAG, "=== Camera  Null===");
        }
    }
...
//
        KLog.i("CJT", angle + " = " + cameraAngle + " = " + nowAngle);
    ...
    public void startRecord(Surface surface, float screenProp, ErrorCallback callback) {
        ...
        KLog.i(TAG, "setVideoSize    width = " + videoSize.width + "height = " + videoSize.height);
...
//        }
        ...
        try {
            ...
            e.printStackTrace();
            KLog.i("CJT", "startRecord IllegalStateException");
            if (this.errorLisenter != null) {
                this.errorLisenter.onError();
            }
        } catch (IOException e) {
            e.printStackTrace();
            KLog.i("CJT", "startRecord IOException");
            if (this.errorLisenter != null) {
                this.errorLisenter.onError();
            }
        } catch (RuntimeException e) {
            KLog.i("CJT", "startRecord RuntimeException");
        }
    }
    ...
    public void handleFocus(final Context context, final float x, final float y, final FocusCallback callback) {
        ...
        if (params.getMaxNumFocusAreas() > 0) {
            ...
            KLog.i(TAG, "focus areas not supported");
